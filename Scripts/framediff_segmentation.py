...
def get_object_start_end_frames(framediff, video, thres=0, outfile=None):
#     """smooth"""
...
#             print "================================================start %i================================================" %fid
        ...
        if (diff == 0 and drawing):  
            ...
            drawing = False
            end_fid = fid
            absdiff = cv2.absdiff(startimg, endimg)
            objmask = pf.fgmask(absdiff, 50, 255, True)
            objbbox = pf.fgbbox(objmask) 
            cv2.rectangle(endimg, (objbbox[0], objbbox[1]), (objbbox[2], objbbox[3]), (255, 255, 255),2)
...
#     print 'fid', fid
    """write object times"""
    if outfile is None:
        outfile = video.videoname + "_framediff_obj_fids.txt"
