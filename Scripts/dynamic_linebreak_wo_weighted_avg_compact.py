...
#             yprojcost = math.pow(5.0, 1.05) + math.pow(yprojcost - 5.0, 0.95)
        ...
        compact = compactcost
        compactcost = math.pow(compactcost, 1.0)
        sum_compactcost = compactcost #sum_compactcost + (numfgpixel * compactcost)
...
#     sum_num_strokes = 0.0
    ...
    avg_compactcost = sum_compactcost/nlines
    sum_strokecost = 0
#     overlap_penalty = 0
...
#     print 'number of objects', len(list_of_objs)
    ...
    outfilename = "linebreak_wo_weighted_avg_compact"
...
#     util.showimages([result])
    ...
    util.write_ints(mybreaker.best_line_id[-1], objdirpath + "/linebreak_wo_weighted_avg_compact_ids.txt")
